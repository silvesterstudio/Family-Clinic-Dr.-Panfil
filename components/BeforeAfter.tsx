...
const cases = [
  { before: "/1-before.webp", after: "/1-after.jpg" },
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-[var(--color-brand-muted)] h-full">
      ...
      <div className="relative h-40 rounded-lg mb-4 overflow-hidden bg-gray-200">
        <Image src={c.before} alt={`Înainte cazul ${index + 1}`} fill className="object-cover" sizes="85vw" loading="lazy" />
      </div>
      <div className="flex justify-start items-center mb-4">
      ...
      <div className="relative h-40 rounded-lg overflow-hidden bg-gray-300">
        <Image src={c.after} alt={`După cazul ${index + 1}`} fill className="object-cover" sizes="85vw" loading="lazy" />
      </div>
    </div>
  );
}

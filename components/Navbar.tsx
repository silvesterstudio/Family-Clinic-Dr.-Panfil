    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        <Link href="#" className="flex items-center gap-2 group shrink-0">
          <Image
            src={isScrolled ? "/logo.svg" : "/white.svg"}
            alt="Clinica Familiei Dr. Panfil"
            width={180}
            height={40}
            className="object-contain w-[88px] sm:w-[115px] md:w-[160px] lg:w-[110px] h-auto transition-all"
          />
        </Link>
        ...
        <div className="hidden md:flex items-center space-x-6">
          <div className={`flex items-center gap-2 font-semibold ${isScrolled ? "text-[var(--color-brand-black)]" : "text-white"}`}>
        ...
        <div className="flex md:hidden items-center gap-3">
          <a href={`tel:${CONTACT_INFO.phone}`} className={`flex items-center gap-1 text-xs font-semibold ${isScrolled ? "text-[var(--color-brand-black)]" : "text-white"}`}>
